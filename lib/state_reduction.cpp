...
#include<iostream>
state::state():PS(' '),NS({'?','?'}), out(-1) {}

void state::input(int index){
...
implicationTable::implicationTable(int n): size(n){
    data.resize(n*(n-1)/2);
}

cell& implicationTable::get(int i, int j){
    if (i < j) std::swap(i, j);
    return data[i*(i-1)/2 + j];


}
int implicationTable::labelToIndex(std::vector<state>& states, char label) {
...
implicationTable& implicationTable::init(std::vector<state>& states){
    for (int i = 1; i < size; i++){
        for (int j = 0; j < i; j++){
            cell& c = implicationTable::get(i, j);
            // check output
...
implicationTable& implicationTable::reduce(std::vector<state>& states){
...
std::vector<std::set<char>> implicationTable::getClass(std::vector<state>& states){
...
char implicationTable::findNewLabel(char oldLabel, std::vector<std::set<char>>& classes,
...
void implicationTable::printReducedTable(std::vector<state>& states){
